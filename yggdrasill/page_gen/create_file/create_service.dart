...
createService(String serviceName) async {
  ...
  serviceTemplateContent =
      serviceTemplateContent.replaceAll("\$ServiceName", serviceName);

  File serviceImplTemplateFile =
      File("./ftr/sample_file/service/service_impl_template");
  String serviceImplTemplateContent =
      await serviceImplTemplateFile.readAsString();
  serviceImplTemplateContent =
      serviceImplTemplateContent.replaceAll("\$ServiceName", serviceName);
  ...
  File createServiceFile = File(path.join(dir.path, '$serviceName.dart'));
  await createServiceFile.create();
  await createServiceFile
      .writeAsString(formatterDartFile(serviceTemplateContent));
  ...
  File createServiceImpFile =
      File(path.join(dirImp.path, '${serviceName}Imp.dart'));
  await createServiceImpFile.create();
  await createServiceImpFile
      .writeAsString(formatterDartFile(serviceImplTemplateContent));
