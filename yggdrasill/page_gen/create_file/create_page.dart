...
_createPageFile(String pageName, String targetDirPath) async {
  ...
  pageTemplateContent = pageTemplateContent
      .replaceAll("\$PageName", pageNameHump)
      .replaceAll("\$PageControllerFileName", pageName + "_controller")
      .replaceAll("\$PageClassName", pageNameHump + "Page");
...
_createControllerFile(String pageName, String targetDirPath) async {
  String pageNameHump = toHumpStr(pageName);
  File controllerTemplateFile =
      File("./page_gen/sample_file/page/controller_template");
  String pageTemplateContent = await controllerTemplateFile.readAsString();
  pageTemplateContent =
      pageTemplateContent.replaceAll("\$PageName", pageNameHump);
  File createControllerFile =
      File(targetDirPath + "/${pageName}_controller.dart");
  await createControllerFile.create();
  await createControllerFile
      .writeAsString(formatterDartFile(pageTemplateContent));
...
_createBindingFile(String pageName, String targetDirPath) async {
  String pageNameHump = toHumpStr(pageName);
  File bindingTemplateFile =
      File("./page_gen/sample_file/page/binding_template");
...
_insertRoute(String originPageName, String targetDirPath) async {
 ...
 class RoutesID {
  ...
  routeFileContent =
      routeFileContent.replaceAll("static final List<GetPage> _routes = [", '''
