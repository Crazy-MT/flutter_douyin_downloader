...
void _printChunked(String message) {
  if (message.isEmpty) {
  ...
  for (final line in message.split('\n')) {
    ...
    for (var index = 0; index < line.length; index += _kLogChunkSize) {
      final end = (index + _kLogChunkSize < line.length)
          ? index + _kLogChunkSize
          : line.length;
      debugPrintSynchronously(line.substring(index, end));
    }
  }
}
...
stackLog({prefixColor = 'b', StackTrace? stackTrace}) {
  ...
  for (var frame in frames) {
    if (frame.location.startsWith("package:flutter") ||
        frame.location.startsWith("dart:")) {
      hideCount++;
      continue;
    }
