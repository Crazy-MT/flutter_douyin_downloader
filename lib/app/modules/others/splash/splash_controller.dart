...
class SplashController extends GetxController {
  ...
  Future<void> onReady() async {
    ...
    if ((Platform.isIOS || Platform.isAndroid) &&
        !(SpUtil.getBool(C.SHOW_DIALOG, defValue: false) ?? false)) {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return ConfirmDialog(
            ...
            confirmText: "同意",
            onConfirm: () {
              SpUtil.putBool(C.SHOW_DIALOG, true);
              initCommon();
            },
            onCancel: () {
              exit(0);
  ...
  RichText richText() {
    return RichText(
      text: TextSpan(
        text:
            "在抖抖资源为您提供服务。在提供服务时需要联网，并基于您的授权调用相机、读取及写入存储器权限，您有权拒绝或取消授权。在使用APP前，请您阅读并充分理解",
        style: TextStyle(
  ...
  Future<void> initCommon() async {
    await common.initCommon();
    opacity.value = 1.0;
    Future.delayed(const Duration(seconds: 1))
        .then((value) => Get.offNamed(RoutesID.DOUYIN_VIDEO_DOWNLOADER_PAGE));
  }
